...
real-ingredient pipeline. Our contributions are:
\begin{enumerate}
...
\citep{Koopmans2006,Auger2010}, which justifies the SIS mapping; however the
mapping carries galaxy-to-galaxy scatter (from density-slope variation and
aperture/anisotropy effects) that propagates into a corresponding scatter in
the training $\thetaE$ labels. Because our ground truth is the SIE $\bsie$ while
our labels are SIS$(\sigmav)$, part of the residual we report is this
irreducible label noise rather than network error. We quantify the direct
SIS$(\sigmav)$-vs-$\bsie$ bias and scatter on the benchmark in
Section~\ref{sec:results} \todo{(E1: report label-noise floor)}, which bounds
the achievable $R^2$.
...
\label{sec:lemon}
We evaluate both methods on the lenses LEMON reports predictions for, in the
Euclidised-HST domain. On the subsample with an independent published $\thetaE$
we lead decisively: SLACS-29 (Bolton $\bsie$) ours $R^2=+0.57$, NMAD $0.045''$,
catastrophic $7\%$ vs LEMON $R^2=-4.26$, NMAD $0.307''$, $55\%$; EELs-12 (Oldham
2017) ours $R^2=+0.83$, NMAD $0.023''$, $8\%$ vs $+0.21$, $0.110''$, $42\%$;
COSMOS-5 is a small-$N$ wash where both struggle (we under-predict 2/5 that lie
above our $\thetaE$ ceiling; disclosed). The ACS-13 subsample has no published
$\thetaE$ (only an arc radius; \citealt{Pawase2014}) and we exclude it from the
$\thetaE$ aggregate. \emph{Reproducibility caveat:} we cannot recover LEMON's
published aggregate (RMSE $0.14''$, NMAD $0.11''$, $R^2=0.53$) by scoring their
released predictions against the literature $\thetaE$; we therefore report our
transparent re-scoring separately and flag the discrepancy
...
\paragraph*{The SIS assumption and label noise.} Our $\thetaE$ labels are
SIS$(\sigmav)$ while the benchmark is SIE $\bsie$; the intrinsic scatter of the
$\sigmav\!\rightarrow\!\thetaE$ relation therefore sets a floor on the
attainable $R^2$ (Section~\ref{sec:massmodel}). This is a feature of any
self-consistent scheme that ties mass to observed kinematics, and it is honest:
we do not claim accuracy below the label-noise floor.
