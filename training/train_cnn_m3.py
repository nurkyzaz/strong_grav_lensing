#!/usr/bin/env python
"""Model 3: same scale-conditioned ResNet as model 2, trained on REALISTIC images
(lens light + varied noise). Adds a noise/lens-light-robust input normalization
(--norm asinh) because per-image min-max lets a bright lens galaxy squash the arc.
This is the project's realistic single-channel model -- NOT Ishida's 2-channel M3."""
import argparse, h5py, numpy as np, torch, torch.nn as nn
from torch.utils.data import Dataset, DataLoader, random_split

PIX_SCALE_M1 = 7.68 / 127
...
def normalize_images(images, norm, asinh_a):
    """Per-image. 'asinh' compresses a bright lens centre while keeping the arc
    distinguishable from noise; 'minmax' is the old model-1/2 behaviour."""
    if norm == "asinh":
...
class LensedScaleDataset(Dataset):
    def __init__(self, image_file, label_file, pixels=128,
                 min_theta_e=0.5, max_theta_e=2.5,
                 norm="asinh", asinh_a=1.0, scale_stats=None):
...
def main():
    ...
    ap.add_argument("--out_ckpt", default="einstein_cnn_m3.pt")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if mae < best:
            ...
            torch.save({"state_dict": model.state_dict(),
                        ...
                        "norm": args.norm, "asinh_a": args.asinh_a}, args.out_ckpt)
