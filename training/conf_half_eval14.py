#!/usr/bin/env python
"""Eval #14 confident-half + recalibrated real-GT coverage."""
import glob
import os
import pandas as pd
import numpy as np
...
for fn in sorted(glob.glob(os.path.join(D, "preds_einstein_cnn_euclid_sel_*_euclid_*.csv"))):
    d = pd.read_csv(fn)
    ...
    tag = os.path.basename(fn).replace("preds_einstein_cnn_euclid_sel_", "").replace("_images.csv", "")
    arch = tag.split("_euclid")[0]
    line = "%-30s N=%3d fail %2.0f%%" % (tag, len(d), 100 * (frac.abs() > 0.15).mean())
    if scols:
