...
| C37 | **GEN5 real high-z SOURCE sample (Nurkyz preference — replace migrated low-z COSMOS).** Candidates: (a) real Euclid Q1 field galaxies from the SAME observations the lenses were found (domain-matched, Nurkyz's trick); (b) HUDF/CANDELS deep high-z; (c) GREAT3 = COSMOS-derived, NOT new (noted). DRAW candidates → **EYEBALL before wiring in**. Weigh resolution (HST crisp/wrong-depth vs Q1 matched/coarser). Do AFTER the C36 brightness batch | Nurkyz 2026-08-02 | OPEN (after C36 eyeball) | new source builder + eyeball gallery |
