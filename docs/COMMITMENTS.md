...
| C16 | LEMON head-to-head (Q1a–Q3) | Nurkyz 2026-07-13 | IN-PROGRESS: Q2 arm complete (⛔ #24 + #25); **ON EMAIL: exact-29 row-filter (no run), 31-lens MAST fetch → euclidise → single ⛔ eval, head-to-head table** | workstream 1 |
...
| C19 | ⛔ eval #25 texture-fixed Q1 re-run + pre-registered interpretation rule (retract-or-confirm #24's headline) | #24 forensics | SUBMITTED 2026-07-14 (job 48007) — harvest, log, apply the rule | DECISIONS_LOG |
| C20 | Path A Rung 0 submission: exactly-once unlabeled run (job 48008) → CSVs (3band all6 primary ×0.98σ, f106 all6 ×1.11σ) → **NURKYZ emails Stony Brook**; log the submission + their returned grade when it arrives | Nurkyz go 2026-07-14 | SUBMITTED — CSVs to Mac on completion | roman_dc/ + DECISIONS_LOG |
