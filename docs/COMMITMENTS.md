...
| C8 | Emails: Busillo — **email REWRITTEN 2026-08-01 (Nurkyz): two concrete asks (1) ACS θ_E ground truth — is it the Pawase arc radius or something else; (2) SLACS — share our preds + ask them to confirm their scoring (our recompute of their SLACS preds vs Bolton gives R²≈−4.3) + permission for a same-lens SLACS comparison in our paper; plus a forward-looking native-Euclid cross-check after GEN5.** Ready to send (Nurkyz to set provenance line + send). Bergamini (HST2EUCLID), TinyLensGPU authors, Brian — still to send | 2026-07-10, upd. 07-13/07-22/08-01 | IN-PROGRESS | EMAIL_DRAFTS_20260710.md |
...
| C19 | **Native-Euclid LEMON head-to-head with the GEN5 model** (Nurkyz directive 2026-08-01): once GEN5 is finalized, compare our model to LEMON on native Euclid data in *their* home domain (the domain-B half of C16). Blocked on GEN5 completion. | Nurkyz 2026-08-01 | OPEN (blocked on GEN5) | MASTER_PLAN §2 domain-B + email C8 point 3 |
| C20 | **ACS scored against Pawase arc radius for BOTH methods (DONE 2026-08-01)** — analysis/lemon_acs_arcradius.py, results/lemon_vs_ours_acs_arcradius.csv, tables/pawase_arc_radius.csv. Reported SEPARATELY from the θ_E aggregate (arc radius ≠ θ_E); paper §4.6 + package README updated. | Nurkyz 2026-08-01 | DONE | PAPER_DRAFT §4.6, lemon_comparison_package/README.md |
