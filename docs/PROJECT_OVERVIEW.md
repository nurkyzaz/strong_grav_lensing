...
**Where this sits.** theta_E prediction is the first of three possible "helper"
CNNs: (1) separate the foreground lens light (hardest),
...
- **Phase 1 (current): standalone CNN.**
  ...
  - Ground-based generalization (HSCempty-style arc injection into real cutouts) —
    planned/future. Extends the CNN beyond clean HST-like sims toward real HSC/CFHT images.
...
Note: the project has since evolved into a standalone θ_E CNN benchmarked on real lenses
(HST and Euclid), with the GEN4/GEN5 hybrid-real training data. See
[HOW_IT_WORKS.md](HOW_IT_WORKS.md) for the current pipeline, model, and how to run it.
...
## Where details live
- How it all works + how to run (GEN4/GEN5, CNN, uncertainty) -> HOW_IT_WORKS.md
- Generator internals, units, file-by-file map -> GENERATOR_AND_CODEBASE_REFERENCE.md
- The GEN5 (Euclid) recipe, stage by stage -> GEN5_PIPELINE.md
- Repo / code locations (older Phase-2 tree) -> CODEBASE_MAP.md
- Running jobs on the cluster -> LensFusion_cluster_runbook.md