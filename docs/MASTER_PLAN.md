...
- **Q1a sample identity**: EELs 13/13 IDENTIFIED (Oldham 2017 Table 2 —
  ...
  (skeleton exists).
- **Q1b imaging (31 non-SLACS)**: DONE 07-15 (30/30 fetched, frozen,
  euclidised, ⛔ eval harvested — see log). **RULED 07-21 after Nurkyz's
  cutout quality review (lemon_review_results.csv): primary table =
  real-θ_E rows only (SLACS-29 + EEL-12/13 + COSMOS-3 ≈ 44; ACS-13
  excluded — Pawase GT is arc-radius/absent and 12/13 images failed
  review; COSMOS 0047+5023 and 0211+1139 dropped on quality/support).
  Pure row-filter on banked per-lens CSVs — no new eval. Disclose ACS in
  the honesty box (13/60 of LEMON's own aggregate GT).**
